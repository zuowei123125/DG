...
// canvasSizeInCM 函数
function canvasSizeInCM(doc, widthInCM) {
    try {
        var resolutionDPI = doc.resolution; // 获取文档的分辨率（每英寸的像素数）

        // 将厘米宽度转换为像素宽度，根据文档的分辨率
        var widthInInches = cmToInches(widthInCM);
        var widthInPixels = widthInInches * resolutionDPI;

        var d = new ActionDescriptor();
        d.putBoolean(stringIDToTypeID("relative"), false); // 设置为绝对值
        d.putUnitDouble(stringIDToTypeID("width"), stringIDToTypeID("pixelsUnit"), widthInPixels);
        d.putUnitDouble(stringIDToTypeID("height"), stringIDToTypeID("pixelsUnit"), doc.height); // 保持高度不变
        d.putEnumerated(stringIDToTypeID("horizontal"), stringIDToTypeID("horizontalLocation"), stringIDToTypeID("center"));
        d.putEnumerated(stringIDToTypeID("vertical"), stringIDToTypeID("verticalLocation"), stringIDToTypeID("center"));
        executeAction(stringIDToTypeID("canvasSize"), d, DialogModes.NO);
    } catch (e) {
...
// 使用示例
ok.onClick = function () {
    var widthInCM = parseFloat(edittext1.text); // 从文本框获取用户输入的宽度
    ...
    for (var i = 0; i < app.documents.length; i++) {
        ...
        if (doc != app.activeDocument) {
            ...
            canvasSizeInCM(doc, widthInCM); // 调用函数来设置宽度
        }
    }
